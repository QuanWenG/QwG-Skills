...
- 复杂项目必须采用“小架构先行”布局法：
  1. 先为每个架构域分别生成独立小图数据，例如服务中心、中间件、存储、CI/CD、外部系统、核心业务链路。
  2. 每个小图先使用极大间距摆放节点、容器和标签，完成包含关系、箭头方向、标签避让和几何自检。
  3. 小图通过后，只在安全范围内逐步收缩到合适大小，不在拥挤初稿里直接微调。
  4. 所有小架构通过后，再组合成项目完整大架构；组合过程也先使用极大间距，再按自检结果逐步收缩。
  5. 记录哪些区域已通过小图自检，哪些跨域连线或业务链路仍需在大图中确认。
...
### 2.2 重写 `src/data/flowTemplate.js`
...
3. 单个小架构必须通过几何自检后才允许收缩：容器完整包含、容器标题不遮挡、普通节点不重叠、标签不压线不压节点、边引用有效。
4. 小架构收缩到合适尺寸后，才把多个小架构合并为完整项目大图；合并时先给架构域之间和跨域箭头保留极大间距。
5. 大图合并也必须重复“极大间距 -> 几何自检 -> 局部收缩”的流程；跨域线、共享中间件、存储和 CI/CD 区域是重点检查对象。
6. 如果大图收缩后再次出现遮挡或交叉密集，回退到上一版安全间距，优先调整分区位置或拆图，不在局部拥挤处硬挪几个点。
...
- 复杂项目不要直接把所有节点塞进一张图里排版；先分别生成并检查各个小架构，再按同样的极大间距收缩方法组合成完整大架构。
